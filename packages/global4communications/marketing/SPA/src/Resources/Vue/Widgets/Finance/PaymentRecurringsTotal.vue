<template>
    <stat-panel :loading="loading">
        ...
        <payment-recurrings-dialog :display_dialog="dialog"
                                   :init_query="query"
                                   @closePaymentRecurringsDrilldown="dialog = false"></payment-recurrings-dialog>

        <v-card-title class="pa-0">Recurring Card Agreements</v-card-title>

        <v-tooltip>
            <template v-slot:activator="{ on, attrs }">
                <v-card-subtitle class="pa-0" v-on="on">{{displaySubTitle}}
                </v-card-subtitle>
            </template>
        </v-tooltip>
...
<script>
    ...
    import PaymentRecurringsDialog from "../../Drilldowns/Finance/PaymentRecurringsDialog";

    export default {
        name: 'payment-recurrings-total',
        template: 'payment-recurrings-total',
        components: {PaymentRecurringsDialog, StatPanel, StatNumber},
        mixins: [stat_panel],
        ...
        methods: {
            ...
            getStatistics() {
                ...
                let request = {
                    endpoint: 'stats_payment_recurrings_total',
                    fields: ['stats'],
                    params: params
                }
                ...
                this.$graph.getEndPointQuery(request)
                    .then((response) => {
                        console.log('Stats Payment Recurrings', response);
