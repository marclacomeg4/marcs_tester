<template>
    <stat-panel :loading="loading">
        ...
        <prospect-dialog :display_dialog="dialog"
                       :init_query="query"
                       @closeProspectsDrilldown="dialog = false"></prospect-dialog>

        <v-card-title class="pa-0">Prospects Created</v-card-title>

        <v-tooltip>
            <template v-slot:activator="{ on, attrs }">
                <v-card-subtitle v-on="on" class="pa-0">Potential Customers</v-card-subtitle>
            </template>
            <span>Potential customers that have been created today</span>
        </v-tooltip>
...
<script>
    ...
    import ProspectDialog from "../../Drilldowns/ResidentialSales/Customers/ProspectsDialog";

    export default {
        name: 'prospects-created-total',
        components: {ProspectDialog},
        template: 'prospects-created-total',
        mixins: [stat_panel],
        props: {},
        data() {
            return {
        ...
        methods: {
            ...
            requestDrilldown(brand) {
                ...
                this.query.params.prospect_only = true;
            ...
            getStatistics() {
                ...
                let request = {
                    endpoint: 'stats_prospects_created_total',
                    fields: ['stats'],
                    params: {
                ...
                this.$graph.getEndPointQuery(request)
                    .then((response) => {
                        console.log('stats_prospects_created_total', response);
