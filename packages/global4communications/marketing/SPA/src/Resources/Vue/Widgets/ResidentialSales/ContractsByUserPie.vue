<template>
    <stat-panel :loading="loading">
        <v-card-title class="pa-0">Contracts by User</v-card-title>
...
<script>
    ...
    export default {
        name: 'contracts-by-user-pie',
        template: 'contracts-by-user-pie',
        ...
        methods: {
            getStatistics() {
                ...
                let request = {
                    endpoint: 'stats_contracts_by_user',
                    fields: ['stats'],
                    params: {
                ...
                this.$graph.getEndPointQuery(request)
                    .then((response) => {
                        console.log('CDR Contracts by User', response);
